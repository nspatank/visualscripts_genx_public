...
suppressMessages ( library ( "RColorBrewer", lib.loc=.libPaths() ) )      # color palletts
suppressMessages ( library ( "tidyverse", lib.loc=.libPaths() ) ) 
library(reshape) # reshaping 
library(ggrepel)
library(scales)
...
dir.create((paste0(directory, "/transmission")), showWarnings = FALSE)
...
for(l in 1:length(cases)){
  ...
  for(j in 1:length(years)) {
    ...
    pjm_base + new_scale("color") + geom_segment(data=EERDWN.pjm.transmission, 
      ...
      scale_color_gradient(low = "light gray", high = "black", labels = percent, name="% New Build") +
      ggtitle(paste0(cases[l], "_", years[j])) +
      labs(size = "Capacity (GW)")+
      ggsave(paste0(directory, "/transmission/", cases[l], years[j], "_transmission_map.png"), width=10, height=10, dpi=300)
